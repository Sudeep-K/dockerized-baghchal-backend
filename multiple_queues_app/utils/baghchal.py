...
class Board:
    ...
    def make_move(self, row, col, kill=False, killrow=None, killcol=None):
        ...
        if (board.player_turn == board.player_goat) and (board.goats["onHand"] > 0):
            ...
            board.position[row][col] = board.player_goat
            board.position[board.selected_position[0]][
                board.selected_position[1]
            ] = board.empty_space
        elif kill:
        ...
        board.player_turn = (
            board.player_goat
            if board.player_turn == board.player_tiger
            else board.player_tiger
        )
        return board
    ...
    def valid_strategies(self):
        ...
        # check valid moves for goat to reposition the goat after all goats are placed on board
            ...
            (self.player_turn == self.player_goat)
            and (self.goats["onHand"] <= 0)
            and (
                self.position[self.selected_position[0]][self.selected_position[1]]
                == self.player_goat
            )
        ):
                        ...
                        if (
                            ...
                            if self.position[newKillRow][newKillCol] == None:
                                self.valid_moves.append(
                                    self.make_move(
                                        newKillRow, newKillCol, True, newRow, newCol
                                    )
                                )
            return 0
            ...
            for empty_space in empty_spaces:
                ...
                for direction in directionChoice:
                    ...
                    if (
                        ...
                        and newCol >= 0
                        and newCol <= 4
                        and self.position[newRow][newCol] == self.player_goat
                    ):
